  ...
  return new Response("OK", { status: 200 });
    ...
    if (text === "/user") {
      try {
        ...
        await this.sendMessage(chatId, `*Total Pengguna Bot:* ${totalUsers} pengguna`, { parse_mode: "Markdown" });
      } catch (error) {
        await this.sendMessage(chatId, `⚠️ Gagal mengambil data pengguna. Error: ${error.message}`, { parse_mode: "Markdown" });
      }
      return new Response("OK", { status: 200 });
    }
    if (text === "/stats") {
...
return new Response("OK", { status: 200 });
  ...
  async handleProxyipCommand(chatId) {
    ...
    try {
      ...
      const ipList = ipText.split("\n").filter((line) => line.trim() !== "");
      if (ipList.length === 0) {
        await this.editMessageText(`⚠️ *Daftar IP kosong atau tidak ditemukan. Coba lagi nanti.*`, { chat_id: chatId, message_id: loadingMessage.result.message_id });
        return;
      }
      ...
      await this.deleteMessage(chatId, loadingMessage.result.message_id);
      await this.sendMessage(chatId, "🌍 *Pilih negara:*", {
        parse_mode: "Markdown",
        reply_markup: { inline_keyboard: buttons }
      });
    } catch (error) {
      console.error("Error fetching IP list:", error);
      await this.editMessageText(`⚠️ *Terjadi kesalahan saat mengambil daftar IP: ${error.message}*`, { chat_id: chatId, message_id: loadingMessage.result.message_id });
    }
  }
};
